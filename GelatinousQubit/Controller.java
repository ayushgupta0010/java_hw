...
/**
 * Set up the GUI and handles events, and lets the person and
 * monster take turns moving.
 */
public class Controller extends Frame implements Runnable {
    ...
    // instance "variables" -- constants for use anywhere
    ...
    private String monsterName = Monster.getName();

    /**
     * Constructs a Controller that uses the given model and view.
     */
    Controller(Model model, View view) {
        String title = MessageFormat.format("{0}!", monsterName);
    ...
    private void step() {
        ...
        if (model.gameIsOver) {
            if (model.personIsAlive)
                messageLabel.setText("YOU HAVE ESCAPED!");
            else
                messageLabel.setText("THE " + monsterName.toUpperCase() + " CONTENTEDLY DISSOLVES YOU AFTER " +
    ...
    class StepButtonHandler implements ActionListener {
         ...
         * person hunt is advanced one step. While single stepping,
    ...
    class ResetButtonHandler implements ActionListener {
         ...
         * bushes and creatures. Cannot be performed while the
